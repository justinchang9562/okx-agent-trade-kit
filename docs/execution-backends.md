...
`POSITION_UNPROTECTED`. The verified `spot_get_fills` schema supports order-ID,
cursor, time-window and archive fields. Recovery prefers exact protective-order
lookup, otherwise uses bounded pagination (20 pages, recent three-day or archive
up to 90-day windows), deduplicates fill IDs, aggregates partial exits and keeps
fees separated by currency. Incomplete history returns a stable fail-closed code
and cannot close the managed position. Transport failure after submission is never blindly
...
Backtests launch a separate `okx-trade-mcp --demo --read-only --modules market`
process without credentials, wrapped by a write-disabled adapter. It is never
the execution backend and cannot place or cancel orders.
